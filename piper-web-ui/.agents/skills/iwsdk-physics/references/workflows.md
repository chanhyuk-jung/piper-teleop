...
```typescript
import { Mesh, SphereGeometry, MeshStandardMaterial, Color, FrontSide } from "three";
import {
  PhysicsShape,
  PhysicsShapeType,
  PhysicsBody,
...
for (const target of this.queries.physicsObjects.entities) {
  ...
  if (dist < radius && dist > 0) {
    const direction = target.object3D.position.clone().sub(explosionPos).normalize();
...
import { Mesh, BoxGeometry, SphereGeometry, MeshStandardMaterial, Color, FrontSide } from "three";
